...
def get_global_rank(parallel_config: ParallelConfig | None = None) -> int:
    """Returns a globally unique rank for the current worker across all parallel
     ...
     ``data_parallel_index`` always keeps the true DP rank (it is never rest),
    ...
    # Number of NPUs ins a single DP replica (TP * PP * prefill-CP)
    replica_size = (
        parallel_config.tensor_parallel_replicas
