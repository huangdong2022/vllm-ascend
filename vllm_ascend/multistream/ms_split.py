...
def model_input_split_v1_attn(
    ...
    if attn_metadata is None:
        return [attn_metadata]
    [token_index,
     seq_index] = compute_split_seq_index(attn_metadata.query_lens,           # tensor([5], dtype=torch.int32)
                                          attn_metadata.attn_state,           # <AscendAttentionState.PrefillNoCache: 0>
                                          attn_metadata.num_actual_tokens)    # 5
    # token_index:0,seq_index:0, len:1
    if token_index == 0 or seq_index == 0 or seq_index == len(
            attn_metadata.query_lens):
        return [attn_metadata]
